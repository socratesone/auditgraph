...
- [ ] T001 Create auditgraph/neo4j/ module directory with __init__.py
- [ ] T002 Add initial neo4j-driver dependency entry to requirements-dev.txt for development and test execution
- [ ] T003 [P] Create exports/neo4j/ default export directory
- [ ] T004 [P] Create tests/fixtures/neo4j_fixtures.py for test graph data
...
- [ ] T005 Implement GraphNodeRecord dataclass in auditgraph/neo4j/records.py with id, type, neo4j_label, name, canonical_key, profile, run_id, source_path, source_hash fields
- [ ] T006 Implement GraphRelationshipRecord dataclass in auditgraph/neo4j/records.py with id, from_id, to_id, type, rule_id, confidence, authority, evidence fields
- [ ] T007 Implement load_graph_nodes() function in auditgraph/neo4j/records.py that loads entities from pkg_root, applies redaction, sorts by id deterministically
- [ ] T008 Implement load_graph_relationships() function in auditgraph/neo4j/records.py that loads links from pkg_root, applies redaction, filters missing nodes, sorts by (from_id, to_id, id) deterministically
- [ ] T009 Implement map_entity_type_to_label() function in auditgraph/neo4j/records.py that converts entity type to `:Auditgraph<Type>` label format
- [ ] T010 [P] Create test_neo4j_records.py with tests for deterministic ordering, redaction application, label mapping
- [ ] T011 Verify all tests in test_neo4j_records.py pass
...
- [ ] T012 [P] [US1] Create ExportSummary dataclass in auditgraph/neo4j/export.py with mode, profile, timestamp, output_path, nodes_processed, relationships_processed, skipped_count, failed_count, duration_seconds fields
- [ ] T013 [P] [US1] Implement generate_export_header() function in auditgraph/neo4j/cypher_builder.py that creates file header comment with profile, timestamp, counts, format version
- [ ] T014 [P] [US1] Implement generate_constraint_statements() function in auditgraph/neo4j/cypher_builder.py that creates unique constraint Cypher for all discovered labels
- [ ] T015 [US1] Implement generate_node_merge_statement() function in auditgraph/neo4j/cypher_builder.py that creates parameterized MERGE with SET for single GraphNodeRecord
- [ ] T016 [US1] Implement generate_relationship_merge_statement() function in auditgraph/neo4j/cypher_builder.py that creates MATCH + MERGE for single GraphRelationshipRecord
- [ ] T017 [US1] Implement batch_records() utility in auditgraph/neo4j/cypher_builder.py that yields 1000-record batches from iterable
- [ ] T018 [US1] Implement write_batched_cypher_file() function in auditgraph/neo4j/export.py that writes header, constraints, nodes (batched with `:begin/:commit`), relationships (batched), to output path and handles file I/O errors with actionable diagnostics
...
- [ ] T020 [US1] Add export-neo4j command to auditgraph/cli.py with --root, --config, --output arguments; default output path to exports/neo4j/<profile>-<run_id>.cypher when --output is omitted
- [ ] T021 [P] [US1] Create test_neo4j_cypher_builder.py with tests for statement generation, batching, deterministic output
- [ ] T022 [P] [US1] Create test_neo4j_export.py with tests for full export flow, deterministic ordering (SC-002), file format validation
- [ ] T023 [US1] Verify export command produces valid .cypher files with correct batching and ordering
...
- [ ] T025 [P] [US2] Create Neo4jConnectionProfile dataclass in auditgraph/neo4j/connection.py with uri, user, password, database, max_connection_pool_size fields
- [ ] T026 [US2] Implement load_connection_from_env() function in auditgraph/neo4j/connection.py that reads NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, NEO4J_DATABASE from environment and validates
- [ ] T027 [US2] Implement create_driver() function in auditgraph/neo4j/connection.py that creates neo4j.GraphDatabase.driver from connection profile
- [ ] T028 [US2] Implement ping_connection() function in auditgraph/neo4j/connection.py that runs simple query to verify connectivity
- [ ] T029 [US2] Implement ensure_constraints() function in auditgraph/neo4j/sync.py that executes CREATE CONSTRAINT IF NOT EXISTS statements for all node labels
- [ ] T030 [US2] Implement sync_nodes_batch() function in auditgraph/neo4j/sync.py that executes parameterized MERGE statements in write transaction for 1000-node batch
- [ ] T031 [US2] Implement sync_relationships_batch() function in auditgraph/neo4j/sync.py that executes MATCH + MERGE statements in write transaction for 1000-relationship batch
- [ ] T032 [US2] Implement sync_neo4j() main function in auditgraph/neo4j/sync.py that orchestrates: load connection, ping, ensure constraints, batch sync nodes, batch sync relationships, return ExportSummary
- [ ] T033 [US2] Add sync-neo4j command to auditgraph/cli.py with --root, --config arguments (reads env vars for connection)
- [ ] T034 [P] [US2] Create test_neo4j_connection.py with tests for env var loading, validation, driver creation, error cases
- [ ] T035 [P] [US2] Create test_neo4j_sync.py with tests for constraint creation, batch sync, idempotency (SC-005), skip logic for missing nodes
- [ ] T036 [US2] Verify sync command creates and updates records idempotently without duplicates
...
- [ ] T037 [US3] Add dry_run parameter to sync_nodes_batch() and sync_relationships_batch() in auditgraph/neo4j/sync.py that skips tx.commit() when enabled
- [ ] T038 [US3] Update sync_neo4j() in auditgraph/neo4j/sync.py to accept dry_run parameter and set mode in ExportSummary
- [ ] T039 [US3] Add --dry-run flag to sync-neo4j command in auditgraph/cli.py
- [ ] T040 [US3] Implement map_neo4j_exception() function in auditgraph/neo4j/connection.py that converts ServiceUnavailable, AuthError, TransientError, ClientError to actionable error messages with diagnostic hints (check NEO4J_URI, verify credentials, etc.)
- [ ] T041 [US3] Wrap driver operations in sync_neo4j() with try/except blocks that use map_neo4j_exception() for error handling
- [ ] T042 [US3] Add error tracking to ExportSummary: add errors field (list of dicts with message and affected record IDs)
- [ ] T043 [US3] Update CLI output formatting in cli.py to display ExportSummary with counts (processed, created, updated, skipped, failed) and duration
- [ ] T044 [P] [US3] Add tests to test_neo4j_sync.py for dry-run mode (no commits), error handling (connection failures, auth errors), summary reporting
...
- [ ] T047 [P] Finalize dependency lock by pinning neo4j-driver to `neo4j>=5,<6` in requirements-dev.txt and removing any unbounded temporary entry from T002
...
- [ ] T052 Run full test suite: pytest tests/test_neo4j_*.py
...
**Checkpoint**: All tasks complete, feature ready for PR
...
# Then sequential:
T048 (performance) → T049 (idempotency overhead) → T052 (test suite) → T053 (quickstart) → T054 (UAT) → T055 (reliability evidence) → T056 (CHANGELOG)
```
...
5. **Polish** (Phase 6) → T046-T056 complete → Final release → ~2 hours
...
4. **Together**: Polish (T046-T056) - ~2 hours
