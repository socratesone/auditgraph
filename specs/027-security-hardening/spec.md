...
- **FR-007**: The MCP server MUST enforce a size cap on string parameter values via `maxLength` constraints declared per-parameter in the tool manifest. Parameters without an explicit `maxLength` MUST inherit a server-level default of **4096 characters** to bound the attack surface of argv construction and to protect against resource-exhaustion probes. The default MUST be overridable at server start via the `AUDITGRAPH_MCP_MAX_STRING_LENGTH` environment variable (accepting any positive integer); the server MUST validate the override at startup and refuse to start if the value is zero, negative, or non-numeric. The default MUST NOT be disableable: there is no way to turn the cap off entirely, only to change its value.
...
- **FR-009**: The validation layer MUST be unit-tested against every tool declared in the tool manifest. Each tool gets at minimum three parametrized test cases: unknown-key payload, type-mismatched payload, and oversized-string payload. A separate inventory-guard test MUST load the manifest at test time and assert `{tool["name"] for tool in manifest["tools"]} == EXPECTED_TESTED_TOOLS`, where `EXPECTED_TESTED_TOOLS` is an explicit module-level constant in the test file. Adding a new tool to the manifest without updating the constant MUST fail this inventory test. Merely relying on `pytest.parametrize` to discover tools dynamically is insufficient because parametrized cases may silently pass on a tool whose schema happens to accept the three adversarial payloads.
...
- **FR-023a**: The system MUST support a `--require-tls` flag on both `auditgraph sync-neo4j` and `auditgraph export-neo4j` that escalates the warning to a hard refusal: non-localhost connections with an unencrypted scheme fail with **exit code 4** (see `contracts/cli-commands.md` for the full exit code allocation table). The environment variable `AUDITGRAPH_REQUIRE_TLS=1` MUST be honored as equivalent to the CLI flag so compliance-bound deployments can enforce strict mode without modifying every invocation.
...
- **FR-026**: The postcondition MUST fail the run on any detected miss by default, exiting the process with **exit code 3** (see `contracts/cli-commands.md` for the full exit code allocation table). The manifest MUST record a structured `redaction_postcondition` entry with shape `{"status": "pass|fail|tolerated", "misses": [...], "allow_misses": bool}` identifying the offending shard(s) and detector category. The `misses` list MUST NOT echo the matched secret value, only the file path and detector category.
- **FR-027**: The system MUST provide a `--allow-redaction-misses` flag on `auditgraph rebuild` that completes the run with `status: tolerated` instead of failing, so operators can complete emergency rebuilds when the postcondition is blocking for a reason they cannot immediately fix. The flag name is exactly `--allow-redaction-misses` (verbosely-named to avoid accidental use in shell history) and MUST be visible in the run manifest via the `allow_misses: true` field so the bypass decision is auditable.
- **FR-028**: The postcondition MUST complete in time proportional to the shard count. The postcondition's additional wallclock MUST NOT exceed the baseline rebuild wallclock measured on the same workspace without the postcondition: formally, `postcondition_wallclock_ms <= baseline_rebuild_wallclock_ms`. This invariant MUST be validated by a paired-measurement test that (a) runs a rebuild against a representative multi-hundred-file corpus with the postcondition disabled and records the wallclock, then (b) runs the postcondition alone against the same resulting `.pkg/` and records its wallclock, then (c) asserts the inequality. A single hardcoded wallclock ceiling is insufficient because it fails to track the underlying ratio as the baseline grows or shrinks.
...
- **SC-008**: The redaction postcondition catches a manual injection of `password=test_sentinel` into a freshly-written chunk file and fails the pipeline run with exit code 3. A paired-measurement test on a representative multi-hundred-file corpus (at minimum 500 files to give measurable numerators and denominators) asserts `postcondition_wallclock_ms <= baseline_rebuild_wallclock_ms`, where `baseline_rebuild_wallclock_ms` is measured on a rebuild of the same workspace with the postcondition disabled. The test MUST include a safety margin for measurement noise (e.g., tolerate up to 10% jitter on the inequality) but MUST NOT tolerate systematic doubling.
- **SC-009**: After all Phase 2-4 work ships, the full repository test suite MUST NOT more than double the current post-Phase-1 baseline wallclock measured in the same environment. The baseline is recorded on the machine running the test suite as the wallclock of a full `pytest tests/ -q` run on `main` at the commit immediately before Spec 027 implementation begins (currently the Phase 1 hotfix merge at `215398d`, measured at 27.5 seconds in the reference developer environment on 2026-04-07). The ratio framing is machine-portable and still catches systematic regressions without pinning a specific hardware baseline. The implementer MUST record the baseline wallclock in `quickstart.md` at implementation time and assert the post-Spec-027 suite runs in `<= 2 * baseline`.
