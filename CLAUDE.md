...
## Recent Changes
- 028-markdown-extraction (SHIPPED): Markdown ingestion now produces queryable `ag:section` / `ag:technology` / `ag:reference` sub-entities via `auditgraph/extract/markdown.py`, plus `contains_section` / `mentions_technology` / `references` / `resolves_to_document` link types. BUG-1 cache-skip fixed (orthogonal `source_origin` field on `IngestRecord`), BUG-2 rule-pack validator + shipped stubs (exit 5 on missing/malformed), BUG-3 wall-clock timestamps on manifests (`wall_clock_started_at` / `wall_clock_finished_at` alongside the unchanged deterministic `started_at`), BUG-4 `auditgraph node` resolves `doc_*`/`chk_*`/`ent_*` with structured not-found envelope, BUG-5 throughput warnings (`no_entities_produced` / `empty_index` fire on binary zero-output-given-nonzero-input). **New explicit declaration**: `markdown-it-py[linkify]>=4,<5` — the `[linkify]` extra pulls in `linkify-it-py>=2,<3`, required at runtime for bare-URL detection.
...
- **Markdown sub-entity ID inputs are source-hash-scoped (Spec 028 data-model.md §1.0)**: `ent_<sha256_text(<source_hash>::<type>::<key>[::<order>])>`. The `canonical_key` field on the entity record is human-readable (slug path / normalized token / redacted raw href) and is DIFFERENT from the hash input. Never use `sha256_text(canonical_key)` to recompute an ID.
- **Markdown pruning is type-scoped (Spec 028 FR-016c/d)**: when a markdown source is re-extracted, `_prune_markdown_subentities_for_source` removes ONLY on-disk `ag:section` / `ag:technology` / `ag:reference` entities for that source path, plus their markdown-rule links. Other entity types (`note`, `ner:*`, `commit`, `tag`, etc.) are never pruned by this mechanism. The helper is defined in `auditgraph/pipeline/runner.py` and called from exactly one site inside `run_extract`.
- **`parse_status` vs `source_origin` orthogonality (Spec 028 FR-001/FR-002)**: cache hits are `parse_status="ok"` + `source_origin="cached"`. `skip_reason="unchanged_source_hash"` is observability-only — it no longer gates downstream stages. The `build_source_record` producer rejects impossible combos: `parse_status="failed"` ⇒ `source_origin="fresh"`; `source_origin="cached"` ⇒ `parse_status="ok"`. Invariant I6.
- **Markdown cache-hits preserve rich source metadata (Spec 028 regression)**: in `run_ingest` and `run_import`, the write loop merges the existing on-disk `sources/<source_hash>.json` payload with the minimal cache-hit record metadata, so fields like `frontmatter`, `document`, `segments`, `chunks` survive reruns. Without this merge, the extract stage falls back from the frontmatter title to the filename stem and silently emits a second `note` entity.
- **Markdown extraction parser setup (Spec 028 FR-016h)**: the canonical form is `MarkdownIt("commonmark", {"linkify": True}).enable("linkify")`. BOTH the constructor option AND the rule activation are required — either alone is a silent no-op. `linkify-it-py` must be importable (pulled in via the `markdown-it-py[linkify]` extras pin). The adapter is `auditgraph/extract/markdown.py :: _tokenize` — the only site in the codebase that imports `markdown_it`.
- **Cooccurrence excludes markdown sub-entities on EITHER endpoint (Spec 028 FR-016e)**: `auditgraph/link/rules.py :: build_source_cooccurrence_links` filters candidate pairs where either side is in `EXCLUDED_COOCCURRENCE_TYPES = {ag:section, ag:technology, ag:reference}`. This is an either-endpoint rule, not same-type-pair — a mixed `ag:note + ag:section` pair is ALSO excluded. Invariant I10.
- **`DocumentsIndex` is built from current-run records, not a disk scan (Spec 028 adjustments3 §4)**: `_build_documents_index` joins the normalized ingest manifest against on-disk `documents/<doc_id>.json` payloads on `source_hash`. Stale document artifacts from prior runs (their sources absent from the current manifest) are silently excluded. The helper reads `document_id` from each payload — it never recomputes via `deterministic_document_id`.
- **`wall_clock_now()` is the only wall-clock entry point (Spec 028 US6)**: `auditgraph/storage/hashing.py :: wall_clock_now` is called from exactly two production sites (`_write_stage_manifest` and `run_ingest` / `run_import`'s `build_manifest` call). Tests monkeypatch this symbol to pin time for byte-identity regressions. Do NOT import `datetime.now` directly in the pipeline — use the helper so tests stay deterministic.
- **Markdown opt-out preserves existing entities (Spec 028 FR-016i)**: setting `extraction.markdown.enabled: false` makes BOTH the producer AND the pruning helper inert. Previously-emitted `ag:section` / `ag:technology` / `ag:reference` entities remain on disk. The flag is a pure activation switch, not a cleanup command — users who want a clean slate must delete the markdown-entity files manually or rebuild from scratch with the flag enabled and then disable.
